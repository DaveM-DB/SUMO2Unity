...
public class RoadNetworkEditorWindow : EditorWindow
{
    ...
    private static string LocateSumoDataFolder()
    {
        ...
        while (dir != null)
        {
            string scenarioCandidate = Path.Combine(dir.FullName, "Scenario");
            if (Directory.Exists(scenarioCandidate)) return scenarioCandidate;

            string requiredFilesCandidate = Path.Combine(dir.FullName, "RequiredFiles");
            if (Directory.Exists(requiredFilesCandidate)) return requiredFilesCandidate;

            string legacyCandidate = Path.Combine(dir.FullName, "Sumo2Unity", "Scenario1");
...
public class Sumo2UnityIntegrationWindow : EditorWindow
{
    ...
    private void OnGUI()
    {
        ...
        EditorGUILayout.LabelField(
            "<b>Instructions:</b>\n\n" +
            "1. From the project root run <i>make run-macos</i>.\n" +
            "2. Or run <i>scripts/run_sumo2unity_macos.sh</i>.\n" +
            "3. Select parameters with command-line flags if needed.\n" +
